...
fun angleInRadian(grad: Int, min: Int, sec: Int): Double = (
        (grad * 3600 + min * 60 + sec)*4.8481368110954e-6
        )
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    ...
    val truePercent :Double = percent.toDouble() / 100
    val firstYear = initial *truePercent
    val tmp = initial + firstYear
    val secondYear = tmp * truePercent
    val tmp2 = secondYear + tmp
    val thirdYear = tmp2 *truePercent
    val total = tmp2 + thirdYear
...
fun numberRevert(number: Int): Int {
    ...
    val result = firstDigit * 100 + secondDigit *10 +thirdDigit
    return result
}
